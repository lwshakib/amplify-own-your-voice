...
- **`prefixPaddingMs`** : The amount of audio to include _before_ speech is detected. This "look-back" ensures the model captures the full onset of speech, including the first syllable which may start before the VAD triggers. A value of `0` may cause the beginning of words to be clipped.
