...
function Scene({
  ...
  const random = useMemo(() => splitmix32(initialSeed), [initialSeed])
  ...
  useFrame((_, delta: number) => {
    ...
    mat.uniforms.uTime.value = _.clock.getElapsedTime() * 0.5

    if (mat.uniforms.uOpacity.value < 1) {
      mat.uniforms.uOpacity.value = Math.min(
        1,
        mat.uniforms.uOpacity.value + delta * 2,
      )
    }
    ...
    if (modeRef.current === "manual") {
      ...
      const t = mat.uniforms.uTime.value * 2
    ...
    curInRef.current += (targetIn - curInRef.current) * 0.15
    ...
    mat.uniforms.uAnimation.value += delta * animSpeedRef.current
    mat.uniforms.uInputVolume.value = THREE.MathUtils.lerp(
      mat.uniforms.uInputVolume.value,
      targetIn,
      delta * 4,
    )
    mat.uniforms.uOutputVolume.value = THREE.MathUtils.lerp(
      mat.uniforms.uOutputVolume.value,
      targetOut,
      delta * 4,
    )
    mat.uniforms.uColor1.value.lerp(targetColor1Ref.current, 0.08)
    mat.uniforms.uColor2.value.lerp(targetColor2Ref.current, 0.08)
  })
